...
For [Block et al.], the project also focus on individuals with hepatocellular carcinoma (HCC) in plasma and non-cancer (NC) for the control.


```{r}
mask <- colData(seR)$Disease %in% c('NC', 'HCC')
subseR <- seR[,mask]
colData(subseR)$Tumor <- factor(subseR$Disease == 'HCC', labels = c('no', 'yes'))
```

```{r}
mask <- (colData(seB)$SampleType == 'HCC' & colData(seB)$Microvesicle == 'no') | colData(seB)$SampleType == 'NHC'
subseB <- seB[,mask]
colData(subseB)$Tumor <- factor(subseB$SampleType == 'HCC', labels = c('no', 'yes'))
```
...
dgeR <- DGEList(counts=assay(subseR), genes=rowData(subseR), samples=colData(subseR))
dgeB <- DGEList(counts=assay(subseB), genes=rowData(subseB), samples=colData(subseB))
```
...
```{r}
mask <- filterByExpr(dgeR, group=subseR$Tumor)
sum(mask)
seR.filt <- subseR[mask, ]
dim(seR.filt)
dgeR.filt <- dgeR[mask, , keep.lib.sizes=FALSE]
dim(dgeR.filt)

mask <- filterByExpr(dgeB, group=subseB$Tumor)
sum(mask)
seB.filt <- subseB[mask, ]
dim(seB.filt)
...
with `Tumor` as main explanatory variable and `Age` and `Sex` as
