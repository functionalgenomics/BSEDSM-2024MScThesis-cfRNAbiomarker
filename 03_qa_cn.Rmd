...
```{r}
mean.ageB <- mean(seB$Age, na.rm = TRUE)
is.na.ageB <- which(is.na(seB$Age) == TRUE)
seB$Age[is.na.ageB] <- mean.ageB
```

### Variable 'Sex'

The estimation of missing data for `Sex` covariate uses a KNN imputer (k = 5) based on the counts for MSY genes (genes from the male-specific region of the Y chromosome). Although no MSY genes counts should be present in female samples, it does not hold for all samples in our dataset (maybe due to technical reasons). This led to disregarding the criteria of imputing based on the assignment of male gender to those individuals with count different than 0 for the MSY genes.

Note that from the 34 MSY genes, only 33 are present in our datasets.

```{r}
MSY_genes <- readRDS("MSYgenes.rds")
length(MSY_genes)
mask <- rownames(seB) %in% MSY_genes
sum(mask)

counts.MSY <- seB[mask,]
seB$Sex <- as.factor(seB$Sex)
assay.counts.MSY <- assay(counts.MSY)
dataframe.counts.MSY <- as.data.frame(t(assay.counts.MSY))

dataframe.counts.MSY$Sex <- seB$Sex

data.imputed <- kNN(dataframe.counts.MSY, k=5, variable="Sex")
seB$Sex <- data.imputed$Sex
```
...
For [@Block2022], the project also focus on individuals with hepatocellular carcinoma (HCC) in plasma (i.e. with Microvesicle == 'no') and non-cancer (NC) for the control.
...
```{r}
table(colData(seB)$SampleType)
mask <- (colData(seB)$SampleType == 'HCC' & colData(seB)$Microvesicle == 'no') | colData(seB)$SampleType == 'NHC'
seB.subset <- seB[,mask]
dim(seB.subset)
colData(seB.subset)$Tumor <- factor(seB.subset$SampleType == 'HCC', labels = c('no', 'yes'))
```
...
dgeR <- DGEList(counts=assay(seR.subset), genes=rowData(seR.subset), samples=colData(seR.subset))
dgeB <- DGEList(counts=assay(seB.subset), genes=rowData(seB.subset), samples=colData(seB.subset))
```
