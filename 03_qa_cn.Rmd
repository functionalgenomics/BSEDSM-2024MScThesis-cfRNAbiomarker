...
# Filtering of lowly-expresseB genes and normalization
...
Change the name of the data from articles [@Roskams2022] and [@Vorperian2022] to _training_ and _testing_ for simplicity.
```{r}
...
### Na imputation of Sex and Age variables
...
```{r}
#is.na.ageB_seB <- which(is.na(seB.filt.testing$Age) == TRUE)
#dgeB.filt.testing$samples$Age[is.na.ageB] <- mean.ageB
...
Table \@ref(tab:hypervargenesMtraining.cn) below shows the top-20 hypervariable genes
with lowest prior degrees of freedom for the last model.

```{r hypervargenesMtraining.cn, echo=FALSE}
...
ktab <- kable(dtf[1:20, ], "html", escape=FALSE, row.names=FALSE,
              caption="Top-20 hypervariable genes for the model umor + Sex + Age + SVs.")
