...
```
```{r hypervargenesMtesting, echo=FALSE}
ord <- order(dgeM.filt.testing.PEGASVs$prior.df)
dtf <- data.frame(prior.df=dgeM.filt.testing.PEGASVs$prior.df[ord],
                  BCV=sqrt(dgeM.filt.testing.PEGASVs$tagwise.dispersion[ord]),
                  Gene=dgeM.filt.testing.PEGASVs$genes$Symbol[ord],
                  Description=dgeM.filt.testing.PEGASVs$genes$Description[ord])
ktab <- kable(dtf[1:20, ], "html", escape=FALSE, row.names=FALSE,
              caption="Top-20 hypervariable genes for the model Preeclampsia + SamplingGA + SVs.")
ktab <- kable_styling(ktab, bootstrap_options=c("striped", "hover", "responsive"),
                      fixed_thead=TRUE)
