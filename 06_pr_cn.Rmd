...
DEgenes.training <- readRDS(file.path("_processed_data", 
                                      "DEgenes.training.rds"))
```

Create a subset with the differential expressed genes from the training dataset from @Roskams2022.

```{r}
dgeR.DEgenes <- dgeR.filt.training$counts[DEgenes.training,]
```
...
```{r}
mask <- DEgenes.training %in% rownames(dgeB.filt.testing$counts)

subset.testing <- matrix(0, nrow = length(DEgenes.training), ncol = ncol(dgeB.filt.testing$counts))

subset.testing[mask, ] <- dgeB.filt.testing$counts[DEgenes.training[mask], ]
```
...
model <- randomForest(x = standardized.training, y = as.factor(tumor.training), ntree = 400)
