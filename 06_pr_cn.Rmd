...
xgboostGrid <- expand.grid(max_depth = c(3, 4, 5, 6, 7), nrounds = (1:10)*20,eta = c(0.3,0.4,0.5),gamma = c(0.5),subsample = c(1),min_child_weight = c(1),colsample_bytree = c(0.7))
...
```{r, results = "hide"}
# Add to output
res.testing <- rbind.data.frame(res.testing, c(pROC::auc(as.numeric(y_test), b), FNR(b, y_test), FPR(b, y_test)))
rownames(res.testing)[nrow(res.testing)] <- 'KerasNN'
```
